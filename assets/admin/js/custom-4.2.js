...
function publishSelectedPosts(message) {
    swal({
        text: message,
        icon: "warning",
        buttons: true,
        ...
        if (willDelete) {
            ...
            $.ajax({
                type: "POST",
                url: InfConfig.baseUrl + "/PostController/publishSelectedDrafts",
                data: data,
                success: function (response) {
                    location.reload();
                }
...
$(document).on('click', '.btn_delete_post_tweet_image', function () {
	console.log($(this).data("image-section"));
	var section_id = $(this).data("image-section").valueOf();
    var content = '<a class="btn-select-image" data-toggle="modal" data-target="#image_file_manager" data-image-type="tweets" data-image-section="'+section_id+'">' +
        '<div class="btn-select-image-inner">' +
        '<i class="fa fa-image"></i>' +
        '<button class="btn">' + txt_select_image + '</button>' +
        '</div>' +
        '</a>';
	$('.post-select-image-container[data-image-section="'+section_id+'"]').html(content);
	$('.post-select-image-container[data-image-section="'+section_id+'"]').next('input[name="tweet_images[]"]').val('');
    $("#input_image_url").val('');
});
...
$('#add_recipient').on('click', function() {
    $('.recipient:first()').clone().find("input").val("").end().appendTo('.recipient_list');
});

/**
 * Remove recipient
 */
$('.recipient_list').on('click', '.remove_recipient', function() {
    if($('.recipient_list').find('.recipient').length == 1) return;
