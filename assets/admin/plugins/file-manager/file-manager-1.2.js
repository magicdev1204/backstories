...
$('#image_file_manager').on('show.bs.modal', function (e) {
	...
	if(image_type=="tweets")
	image_sec = $(e.relatedTarget).attr('data-image-section');
});
...
function select_image() {
    ...
    } else if (image_type == 'tweets')  {
        var image = '<div class="post-select-image-container" data-image-section="'+image_sec+'" style="display:block;">' +
            '<img src="' + base_url + img_url + '" alt="">' +
            '<a class="btn btn-danger btn-sm btn-delete-selected-file-image btn_delete_post_tweet_image" data-image-section="'+image_sec+'">' +
            '<i class="fa fa-times"></i> ' +
            '</a>' +
            '</div>';
		$('.post-select-image-container[data-image-section="'+image_sec+'"]').html(image);
        $('.post-select-image-container[data-image-section="'+image_sec+'"]').next('input[name="tweet_images[]"]').val(file_id);
