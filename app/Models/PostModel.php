...
class PostModel extends BaseModel
{
    ...
    public function setFilterQueryPetition()
    {
        ...
        $this->builder->where('posts.petition_approve', 1)->where('posts.visibility', 1)->where('posts.status', 1)->where('posts.lang_id', $this->activeLangId);
    }
    ...
    {
        $this->setFilterQuery();
        return $this->builder->where('posts.filtering', 0)->orderBy('posts.created_at', 'DESC')->limit($perPage, $offset)->get()->getResult();
    }
    ...
    public function getPostCountByCategory($categoryTree)
    {
        if (!empty($categoryTree)) {
            $this->setFilterQuery();
            return $this->builder->whereIn('posts.category_id', $categoryTree, false)->where('posts.filtering', 0)->where('posts.visibility', 1)->where('posts.status', 1)->countAllResults();
        }
        return 0;
    }
    ...
    public function getPostRecipients($postId) {
        $recipients = $this->db->table('recipients')->where('post_id', cleanNumber($postId))->get()->getResult();

        return $recipients;
    }

    public function getPostByShortUrl($shortened_url)
    {
        return $this->builder->where('image_shortened_url', $shortened_url)->get()->getRow();
    }

    public function getPostAdditionalImagesByShortUrl($shortened_url)
    {
        return $this->db->table('post_images')->where('shortened_url', cleanNumber($shortened_url))->get()->getRow();
    }
}
