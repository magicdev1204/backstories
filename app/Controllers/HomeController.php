...
class HomeController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['languageSettings'] = getLanguageSettings();
    ...
    private function post($slug)
    {
        ...
        if (empty($data['post'])) {
            ...
            $postRecipients = $model->getPostRecipients($id);
            $data['postRecipients'] = $postRecipients;
            $postRecipientEmails = '';
            foreach($postRecipients as $recipient) {
                $postRecipientEmails = $postRecipientEmails . $recipient->recipient_email . ',';
            }

            $data['postRecipientEmails'] = $postRecipientEmails;
    ...
    {
        $model = new FileModel();
        $name = $model->getImageByShortUrl($url)->file_name;
    ...
    // from `posts` and `post _images` table
    public function attachmentImage($url)
    {
        $model = new PostModel();

        $name = '';
        if($model->getPostByShortUrl($url)) {
            $name = $model->getPostByShortUrl($url)->image_big;
        } else {
            $name = $model->getPostAdditionalImagesByShortUrl($url)->image_path;
        }
