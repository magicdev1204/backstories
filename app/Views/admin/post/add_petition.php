<div class="row">
    <div class="col-sm-12">
        <form action="<?= base_url('PostController/addPostPost'); ?>" method="post">
            <?= csrf_field(); ?>
            <input type="hidden" name="post_type" value="petition">
            <div class="row">
                <div class="col-sm-12 form-header">
                    <h1 class="form-title"><?= trans('add_petition'); ?></h1>
                ...
                <div class="col-sm-12">
                    <div class="form-post">
                        <div class="form-post-left">
                            <?= view("admin/post/_form_add_petition_left"); ?>
                            <?= view("admin/post/_form_add_recipient"); ?>
                        </div>
                                                ...
                                                <select id="categories" name="category_id" class="form-control" onchange="getSubCategories(this.value);" required>
                                                    ...
                                                    <?php if (!empty($parentCategories)):
                                                        foreach ($parentCategories as $item): ?>
                                                            <option value="<?= esc($item->id); ?>" <?= $item->id == old('category_id') || $item->slug == 'petition' ? 'selected' : ''; ?>><?= esc($item->name); ?></option>
                                                        <?php endforeach;
                                                    endif; ?>
                                                </select>
                                            </div>
