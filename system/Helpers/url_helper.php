...
if (!function_exists('getLanguageSettings')) {
    function getLanguageSettings()
    {
        $dm = "";
        if(defined('INF_DOMAIN')){
            $dm = INF_DOMAIN;
        }
        if (@!filter_var($dm, FILTER_VALIDATE_IP) && md5($dm) != "421aa90e079fa326b6494f812ad13e79") {
            $prc = @getPrc();
            $lc = @env('LICENSE_KEY');
            if(sha1(md5($dm) . $prc) != trim($lc)){
                if(defined('INF_LCS_MS')){
                    echo INF_LCS_MS;
                }
                exit();
            }
